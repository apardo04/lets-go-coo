let robin, hipster, lumberjack, runningHipster, tree, kite, drone, seagull, turd, package, fountain, text, lose, time;
let clickTime = -1000;
let robinWidth = 150;
let robinHeight = 139;
...
let robin_sheet, robin_animation, turd_sheet, turd_animation, hipster_sheet, hipster_animation, lumberjack_sheet, lumberjack_animation, runningHipster_sheet, runningHipster_animation, drone_sheet, drone_animation, seagull_sheet, seagull_animation, kite_sheet, kite_animation, packageImg, textImg, keyboardImg, loseImg, comingsoonImg, errorImg;
let robin_sprite, leia_sprite, hipster_sprite, lumberjack_sprite, runningHipster_sprite, tree_sprite, kite_sprite, drone_sprite, seagull_sprite, turd_sprite, package_sprite, fountain_sprite, text_sprite, keyboard_sprite, leia_bubble_sprite, lose_sprite, comingsoon_sprite;
let fart, grunt;
let objects, trees, flyers, turds, hipsters;
let score = 0;
let readyButton, survivalButton;
...
function preload() {
    ...
    seagull_sheet = loadSpriteSheet('assets/img/seagull_sheet.png',130,140,10);
...
function draw() {
    ...
    for (let i = 0; i < flyers.length; i++) {
        if (flyers[i].position.x + flyers[i].width < 0) {
            ...
            setTimeout(function() {
                flyers[i].setSpeed(-(6+score/10),0);
                //console.log("timeout  flyer | speed="+speed)
            }, rand);
        }
    }
    // Kite Spawning Code
    ...
    if (kite.go) {
        kite_sprite.setSpeed(-(4+score/10), kite.dir);
    ...
    for (let i = 0; i < hipsters.length; i++) {
        ...
        // Turds Overlapping Hipsters Code
...
function gameOver() {
    ...
    readyButton = createButton('Main Menu', 'Main Menu');
    readyButton.position(width / 2 - 150, height / 2 + 100);
    survivalButton = createButton('Try Again!', 'Try Again!');
    survivalButton.position(width / 2 + 40, height / 2 + 100);
